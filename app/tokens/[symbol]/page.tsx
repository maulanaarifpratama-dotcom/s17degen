...
export default function TokenWorldPage() {
  ...
  return (
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,#1e1b4b_0%,#020617_42%,#020617_100%)] text-slate-50">
      ...
      <section className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8"><div className="flex flex-col gap-3 rounded-[2rem] border border-slate-800 bg-slate-900/70 p-6 sm:flex-row"><a href={token.pumpFunLink} target="_blank" rel="noopener noreferrer" onClick={() => window.localStorage.setItem('s17:last-buy-intent', token.symbol)} className="inline-flex flex-1 items-center justify-center gap-2 rounded-full border border-fuchsia-400/40 px-5 py-3 font-black text-fuchsia-100 hover:bg-fuchsia-400/10">Buy on Pump.fun <ExternalLink className="h-4 w-4" /></a><button onClick={refresh} className="flex-1 rounded-full bg-slate-100 px-5 py-3 font-black text-slate-950 hover:bg-cyan-200">Verify Access</button></div></section>
      <Footer />
    </main>
  );
}
